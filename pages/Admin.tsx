      ...
      <div className="min-h-screen bg-brand-black flex items-center justify-center p-4">
        <div className="w-full max-w-md bg-slate-900 border border-white/10 p-8 sm:p-12 shadow-2xl">
          <div className="text-center mb-10">
            ...
            <p className="text-slate-500 text-[10px] font-black uppercase tracking-[0.3em]">Institutional Verification Required</p>
          </div>
          <form onSubmit={handleLogin} className="space-y-6">
            ...
            <div className="space-y-2">
              <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest block">Personnel ID</label>
            ...
            <div className="space-y-2">
              <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest block">Security Key</label>
        ...
        {activeTab === 'dashboard' && (
          <div className="space-y-12 animate-fade-up">
            <header>
              ...
              <p className="text-slate-400 text-xs font-black uppercase tracking-[0.5em]">Real-time Institutional Metrics</p>
            </header>
                ...
                {posts.map(p => (
                  <div key={p.id} className="p-6 bg-slate-50 border border-slate-100 flex justify-between items-center group">
                    ...
                    <div className="flex gap-4 opacity-0 group-hover:opacity-100 transition-opacity">
                      <button onClick={() => { setEditingId(p.id); setPostData(p); setView('form'); }} className="text-slate-400 hover:text-brand-primary"><Edit3 size={18} /></button>
                ...
                {events.map(ev => (
                  <div key={ev.id} className="p-6 bg-slate-50 border border-slate-100 flex justify-between items-center group">
                    <div>
                      ...
                      <p className="text-xs text-slate-400 font-black uppercase tracking-widest">{ev.date} | {ev.location}</p>
                    </div>
                    <div className="flex gap-4 opacity-0 group-hover:opacity-100 transition-opacity">
                      <button onClick={() => { setEditingId(ev.id); setEventData(ev); setView('form'); }} className="text-slate-400 hover:text-brand-primary"><Edit3 size={18} /></button>
            ...
            {view === 'list' ? (
                    ...
                    {users.map(u => (
                      <tr key={u.id} className="hover:bg-slate-50 transition-colors">
                        ...
                        <td className="p-6 text-right space-x-4">
                          <button onClick={() => { setEditingId(u.id); setUserData(u); setView('form'); }} className="text-slate-400 hover:text-brand-primary"><Edit3 size={16} /></button>
              ...
              <form onSubmit={saveUser} className="space-y-8 max-w-4xl bg-slate-50 p-8 border border-slate-100">
                <div className="grid grid-cols-2 gap-8">
                  ...
                  <div className="space-y-2">
                    <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Personnel ID (Username)</label>
        ...
        {activeTab === 'leads' && (
          <div className="space-y-12 animate-fade-up">
            ...
            <div className="space-y-6">
              {leads.map(l => (
                <div key={l.id} className="p-10 bg-slate-50 border border-slate-100 flex flex-col md:flex-row justify-between gap-8 hover:border-brand-primary transition-all">
                  <div className="space-y-4">
                    ...
                    <p className="text-slate-500 italic max-w-2xl leading-relaxed">"{l.message}"</p>
                    <div className="flex gap-6 text-[10px] font-black text-slate-400 uppercase tracking-widest pt-4">
              ...
              {leads.length === 0 && <p className="text-slate-400 italic">No unread briefings available.</p>}
            </div>
          </div>
        )}
      </main>
